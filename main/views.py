...
from .models import AnnotationImageModel, DontFakeMe, YoloClassModel, ImageDetectionModel
...
def detection_image(request):
    if request.FILES:
        ...
        stat = get_client_ip(request)
        model = ImageDetectionModel(image=image, stat=stat)
        return JsonResponse({
            "status": True,
